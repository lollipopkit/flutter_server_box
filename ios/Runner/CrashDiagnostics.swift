...
/// The iOS half of native crash reporting: what the system recorded about a
/// crash this app could not see.
///
/// A SIGSEGV in the Linux engine, in the Rust FFI or in sqlite ends the process
/// outright — no Dart handler runs, nothing is written, and the next launch
/// looks like a clean start. MetricKit is the platform's own record of it.
///
/// Not a signal handler, deliberately. A crash SDK installs one for SIGSEGV and
/// SIGABRT, and this app is a bad place for that: the Linux engine interrupts
/// its guest threads with SIGUSR1 and expects to own signal disposition.
/// MetricKit collects out of process and competes with nothing.
///
...
final class CrashDiagnostics: NSObject, MXMetricManagerSubscriber {
    ...
    /// Enough to explain a pattern, few enough that a device left offline for
