...
`plugin_kv` 提供持久化存储。实例内的临时状态用普通 JavaScript 变量即可，但实例销毁后不保留。键值数据默认留在本机，声明并获准 `storage.sync` 后，按插件 id 作为独立同步项处理。

备份已实现：`BackupV2` 的 `plugins: {id: {cfg: [...], kv: [...]}}`（`PluginBackup`）。两点与 `container` 不同：

- **只增不删。** 备份不是这份数据的完整状态 —— 文件可能来自装了另一组插件的设备，删掉它没提到的东西，会让"恢复一份旧备份"变成丢掉新插件数据的办法。两边指向同一个东西时，文件赢。
- 服务器 id 走 snippet 和 port forward 同一套重映射；指向本机不存在的服务器的条目跳过（两张表都有外键）。

**分享不是备份，也不带这些。** `ServerShare` 是另一个模型，而插件配置可能含 manifest 标了 `secret` 的字段（BMC 密码）。如果将来分享要带插件配置，第七节"分享时按 `secret` 标记移除"就得在那里落实。

卸载时删除插件文件、安装记录并撤销权限，是否保留配置和数据由用户选择。删除服务器时，它关联的插件配置和数据一并清理。已实现：`PluginPackage`（读 `.sbp`，挡住路径穿越和解压炸弹）+ `PluginInstaller`（先落文件、再写记录、最后加进用户的排列 —— 反过来会得到一条指向不存在文件的记录，也就是每次启动都加载失败的插件）。

**安装记录不进备份。** 备份带的是设置和记录、不是文件，恢复出来的记录会指向一个不存在的目录。备份带的是插件的**数据**（下面的 `plugins` 字段），它在插件被重新安装之前一直留着。
...
| `sb.server.exec` | 无 | agent 不发句柄，插件没有东西可以填进 `server` |

**`sb.server.exec` 在 agent 上没有，不是出于信任。** 它按句柄指定服务器，而 agent 既没有绑定的服务器、也没有 `sb.server.list`，所以任何一次调用都只能填一个不存在的句柄。与其接下参数再拒绝，不如说「这个宿主没有这个函数」。状态插件在 `statusCmd` 里说要跑什么、agent 去跑，本来就是同一份能力，只是名字对得上。
...
`packages/plugins/` 下三个都是 `contributes.page` + `onHook` 采集，各自带 MockHost 测试和一个在真 QuickJS 上跑的 Dart 测试（后者证明的是 SDK 和 App 一致，不是插件和 SDK 一致）。

**加进 SDK 的**：`shellQuote`。插件拼进命令的值几乎总是**来自服务器**——列表里的路径、`systemctl` 里的单元名——而 `; rm -rf ~` 是一个合法的目录名。只有一个正确写法，而且很容易写出一个「测试作者想得到的输入都对」的错版本，所以放进 SDK 而不是留给每个插件。

**发现缺的接口，都没有为单个插件加**：

- `sb.server.exec` **不能取消**。`du` 慢是构造性的，只能事先给 `timeoutMs`，不能在一分钟时放弃一次预计四分钟的扫描。
- 没有 `sb.nav.openPortForward`，所以监听端口插件点一条直接转发做不到。为一个插件加一个方法，是宿主接口长成「一个功能一个方法」的路子。

**踩到的行为，值得写进插件文档**：

- `sb.ui.patch` 在 surface 不在屏幕上时会抛。这是对的——流式输出的插件该能知道没人看——但无保护地 `await` 它，就把「没人在看」变成了「活儿停了」：磁盘占用插件第一版画完 `Measuring…` 就抛了，命令根本没跑。
- 权限拒绝**是可以被插件 catch 的**（JavaScript 没有不可捕获的抛出），宿主是在调用结束后让整个 call 失败。安全属性是「App 没有跑那条命令」，不是「插件不知道自己被拒了」。
