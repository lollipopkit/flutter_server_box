...
void main() {
  ...
  group('cancelling', () {
    ...
    test('an inherited output pipe does not hold the result up', () async {
      final result = await exec
          .run(_posixShell('(sleep 30) & printf done'))
          .timeout(
            const Duration(seconds: 5),
...
/// Runs [script] under `sh`, whatever the login shell is.
///
/// [ProcessExec] executes through `LocalShellBackend.shellPath`, which is
/// `$SHELL` on POSIX so that a terminal opens the shell its user chose. Fish is
/// a common one and its parentheses are command substitution, not grouping — so
/// a process-tree fixture written in POSIX syntax does not fail there, it never
/// starts, and the assertion that nothing survived passes having tested
/// nothing. Only the process-tree fixtures need this; every other command here
/// is a whole program.
String _posixShell(String script) => 'sh -c ${shellSingleQuote(script)}';
