...
/// The upgrade path from a real released build, against bytes that build wrote.
///
/// `hive_import_test.dart` seeds its boxes through the *current* adapters,
/// which is the wrong shape for this question: it cannot catch a decoder that
/// disagrees with what the old release actually put on disk. The fixture here
/// was produced by v1.0.1466's own generated adapters — see
/// `test/fixtures/hive_v1466/README.md` — so a mistake in reading it fails
/// here rather than on a user's device, where the pass is not repeatable.
///
/// v1.0.1466 is the build the App Store served while this migration was
/// written, so it is the version most installs upgrade from.
void main() {
  TestWidgetsFlutterBinding.ensureInitialized();

  final fixtureDir = Directory('test/fixtures/hive_v1466');
  late Directory tempDir;

  setUpAll(() async {
    tempDir = await Directory.systemTemp.createTemp('sbm-hive-v1466-');
    Paths.doc = tempDir.path;

    // The key the fixture was written with. Same construction as the
    // generator, so the cipher matches and the boxes open.
    final rng = Random(1);
  ...
  setUp(() async {
    // Fresh every time: the import deletes `conn_stats_index` and opening a
    // box rewrites it, so a test must not run against what the last one left.
  ...
  group('after importing the v1.0.1466 boxes', () {
    setUp(() async => Stores.init());
    ...
    test('the richest record keeps every optional field', () {
      ...
      expect(spi.ssh?.keyId, 'key-ed25519');
    ...
    test('a record with nothing optional set survives', () {
      final bare = Stores.server.fetchOneRaw('srv-bare')!;
    ...
    test('the stored JSON has no flat SSH key left', () {
      ...
      expect((decoded['ssh'] as Map)['ip'], '10.0.0.2');
      for (final flat in const ['ip', 'port', 'user', 'pwd', 'pubKeyId']) {
        expect(decoded.containsKey(flat), isFalse, reason: 'flat "$flat"');
      }
    });

    test('private keys and snippets come across whole', () {
      expect(Stores.key.fetchOneRaw('key-ed25519')?.key,
          contains('BEGIN OPENSSH PRIVATE KEY'));
      expect(Stores.key.fetchOneRaw('key-rsa')?.key,
          contains('BEGIN RSA PRIVATE KEY'));

      final deploy = Stores.snippet.fetchOneRaw('deploy')!;
      ...
      expect(deploy.autoRunOn, ['srv-pwd', 'srv-key']);

      // Non-ASCII, quotes, backslashes and newlines through Hive bytes and
      // then through JSON.
      final unicode = Stores.snippet.fetchOneRaw('unicode')!;
      expect(unicode.name, '日本語 / emoji 🚀');
    ...
    test('history, container hosts and port forwards come across', () {
      ...
      expect(Stores.container.fetch('', ContainerType.docker),
      ...
      final forwards = Stores.portForward.fetch('srv-pwd');
      ...
      expect(Stores.portForward.fetch('srv-key').single.type,
