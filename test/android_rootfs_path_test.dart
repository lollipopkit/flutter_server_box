...
void main() {
  ...
  setUp(() async {
    ...
    await Directory(p.join(root.path, 'etc')).create(recursive: true);
    await File(
      p.join(root.path, 'etc', 'alpine-release'),
    ).writeAsString('3.22.5\n');
  ...
  String inside(String relative) =>
      p.joinAll([realRoot, ...p.posix.split(relative)]);

  group('a path inside the container', () {
    test('is the same path under the rootfs', () async {
      expect(await resolve('/etc/alpine-release'), inside('etc/alpine-release'));
    });
    ...
    test('resolves . and redundant separators', () async {
      expect(
        await resolve('/./etc//alpine-release'),
        inside('etc/alpine-release'),
      );
    });

    test('a file that does not exist yet can still be written', () async {
      expect(
        await resolve('/tmp/new.txt', forWrite: true),
        inside('tmp/new.txt'),
      );
    });
  });
  ...
  group('a path that leaves it', () {
    test('..  is resolved against the container root, not the host', () async {
      ...
      expect(
        await resolve('/etc/../../etc/alpine-release'),
        inside('etc/alpine-release'),
      );
    });
    ...
    test('a symlink out of the container is refused', () async {
      ...
      await File(p.join(outside.path, 'secret')).writeAsString('not yours');
      await Link(p.join(root.path, 'tmp', 'out')).create(outside.path);
    ...
    test('a symlink within the container is followed', () async {
      await Link(
        p.join(root.path, 'tmp', 'release'),
      ).create(inside('etc/alpine-release'));

      expect(await resolve('/tmp/release'), inside('etc/alpine-release'));
    });
  ...
  test('no rootfs on disk means nothing is inside one', () async {
    expect(
      await AndroidRootfs.resolveWithin(p.join(root.path, 'gone'), '/etc/hosts'),
      isNull,
    );
  });
}
