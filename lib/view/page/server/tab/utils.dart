...
extension _Operation on _ServerPageState {
  void _onTapSuspend(Server srv) {
    _askFor(
      func: () async {
        ...
        srv.client?.execWithPwd(ShellFunc.suspend.exec(srv.spi.id, systemType: srv.status.system), context: context, id: srv.id);
      },
      typ: l10n.suspend,
      name: srv.spi.name,
    );
  ...
  void _onTapShutdown(Server srv) {
    _askFor(
      func: () => srv.client?.execWithPwd(ShellFunc.shutdown.exec(srv.spi.id, systemType: srv.status.system), context: context, id: srv.id),
      typ: l10n.shutdown,
      name: srv.spi.name,
    );
  }
  ...
  void _onTapReboot(Server srv) {
    _askFor(
      func: () => srv.client?.execWithPwd(ShellFunc.reboot.exec(srv.spi.id, systemType: srv.status.system), context: context, id: srv.id),
      typ: l10n.reboot,
      name: srv.spi.name,
    );
  }
