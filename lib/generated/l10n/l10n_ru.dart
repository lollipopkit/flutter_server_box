...
class AppLocalizationsRu extends AppLocalizations {
  ...
  String distIconIntroLegal(Object fontLogos) {
    return 'Эти значки взяты из $fontLogos. Значок отражает лишь то, что это устройство прочитало с удалённой системы, и эти сведения могут быть неверными или устаревшими; он не обозначает производную сборку, пересборку или конкретную версию. Если систему определить не удалось или её значок не поставляется, вместо него рисуется нейтральный контур.\n\nКаждый значок является товарным знаком соответствующего владельца и используется здесь только для указания на систему, которую он обозначает.';
  }
