...
- Pages project settings (the live project's **root directory is `monitor/frontend`**, confirmed from a build log — not the repo root): build command `npm install --prefix ../../packages/webui && npm run build`, output directory `dist` (relative to the root directory, so this resolves to `monitor/frontend/dist`)
  - The `npm install --prefix ../../packages/webui` step is required: `@serverbox/webui` is a `file:../../packages/webui` dependency, and Cloudflare's automatic `npm clean-install` (run against the configured root directory only) just symlinks it in — it never installs *webui's own* devDependencies (svelte, clsx, tailwind-variants, ...). Without this step `svelte-check` fails with "Cannot find module" for every file under `packages/webui/src` (same class of bug the Dockerfile hit — see `RUN npm install --prefix /app/packages/webui` there). Verified against a real build log (`svelte-check found 8 errors` without the fix, `0 errors` with it, reproduced locally with root directory = `monitor/frontend` to match the actual project config).
  - If the root directory setting is ever changed to the repo root instead, use `npm install --prefix packages/webui && cd monitor/frontend && npm ci && npm run build` with output directory `monitor/frontend/dist` instead.
  - `frontend/.node-version` is what tells Pages which Node to use; without it the default is 18, and `vite` 8 requires `^20.19.0 || >=22.12.0` — the build dies before any of this repo's code is read. Pages reads it from the **root directory**, so it lives in `monitor/frontend/`, not at the repo root. The Dockerfile (`node:22-alpine`) and `monitor-release.yml` (`node-version: 22`) already agreed on 22; this was the one builder nobody had told.
