...
- Pages project settings (the live project's **root directory is `monitor/frontend`**, confirmed from a build log — not the repo root): build command `npm run build`, output directory `dist` (relative to the root directory, so this resolves to `monitor/frontend/dist`)
  ...
  - Node comes from the project's environment (a build log shows `nodejs@22.22.0`), which is what `vite` 8 needs — it declares `^20.19.0 || >=22.12.0`. No `.node-version` in the repo; if that environment is ever reset to Pages' default the build dies before reading any of this code, and adding one under `monitor/frontend/` is the fix.
