...
const DUMMY_PASSWORD_HASH: &str = "$2b$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxDr5E6a6E9E/sy0tESrQ6D7x8e";
...
pub fn verify_password(password: &str, hash: &str) -> Result<bool> {
    bcrypt::verify(password, hash).map_err(|e| MonitorError::Auth(e.to_string()))
}
...
pub fn verify_login_password(password: &str, hash: Option<&str>) -> Result<bool> {
    let matched =
        verify_password(password, hash.unwrap_or(DUMMY_PASSWORD_HASH)).unwrap_or_default();
    Ok(hash.is_some() && matched)
}

pub fn hash_password(password: &str) -> Result<String> {
    bcrypt::hash(password, bcrypt::DEFAULT_COST).map_err(|e| MonitorError::Auth(e.to_string()))
}
