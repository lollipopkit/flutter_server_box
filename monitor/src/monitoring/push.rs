...
pub async fn send_notification(config: &Config, push: &PushConfig, message: &str) -> Result<()> {
    let _permit = push_limit().clone().acquire_owned().await.map_err(|_| {
        crate::utils::error::MonitorError::Push("Push queue is unavailable".to_string())
    })?;
...
async fn send_webhook_notification(
    ...
    let url = push
        ...
        .and_then(|v| v.as_str())
        .ok_or_else(|| {
            crate::utils::error::MonitorError::Push("Missing webhook URL".to_string())
        })?;
...
async fn send_serverchan_notification(
    config: &Config,
    push: &PushConfig,
    message: &str,
) -> Result<()> {
    ...
    let sc_key = push
        ...
        .and_then(|v| v.as_str())
        .ok_or_else(|| {
            crate::utils::error::MonitorError::Push("Missing ServerChan SCKey".to_string())
        })?;
...
async fn send_ios_notification(config: &Config, push: &PushConfig, message: &str) -> Result<()> {
    ...
    let response = client
        .post("https://push.lolli.tech/v1/ios")
        .json(&body)
    ...
    if let Some(body_regex) = push.config.get("body_regex").and_then(|v| v.as_str()) {
        let re = regex::Regex::new(body_regex).map_err(|e| {
            crate::utils::error::MonitorError::Push(format!("Invalid regex pattern: {}", e))
        })?;

        if !re.is_match(&response_text) {
            warn!(
                "iOS notification response didn't match expected pattern: {}",
                response_text
            );
            return Err(crate::utils::error::MonitorError::Push(format!(
...
fn validate_response(push: &PushConfig, status: u16, body: &str) -> Result<()> {
    ...
    if let Some(pattern) = push.config.get("body_regex").and_then(|v| v.as_str()) {
        let regex = regex::Regex::new(pattern).map_err(|error| {
            crate::utils::error::MonitorError::Push(format!("Invalid regex pattern: {error}"))
        })?;
...
mod tests {
    ...
    fn test_replace_template_string() {
        let result = replace_template_string(
        ...
        assert_eq!(
            replace_template_string("{{msg}}", "Test message", "my-server"),
            "Test message"
        );
    }
    ...
    async fn webhook_accepts_a_small_success_response() {
        ...
        assert!(
            send_notification(&config("test-server"), &push, "Test Message")
                .await
                .is_ok()
        );
    }
    ...
    async fn webhook_reports_an_http_failure() {
        ...
        let error = send_notification(&config("test-server"), &push, "Test Message")
            .await
            .unwrap_err();
    ...
    async fn a_go_webhook_body_and_placeholders_survive_the_request() {
        ...
        push.config
            .insert("expected_http_status".to_string(), TomlValue::Integer(202));
        push.config.insert(
            "body".to_string(),
            toml::toml! {
        ...
        send_notification(&config("legacy-host"), &push, "CPU: 91%")
            .await
            .unwrap();
        let request = String::from_utf8(request.await.unwrap()).unwrap();
        assert!(request.starts_with("POST / HTTP/1.1"));
        assert!(
            request.contains("\"message\":\"legacy-host CPU: 91%\""),
            "request was {request}"
        );
    }
