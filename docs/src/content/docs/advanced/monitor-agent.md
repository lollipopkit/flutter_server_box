...
Part of that file can be edited without opening it. Collection intervals, alert rules, notification channels, data retention and the allowed panel origins are editable from the web panel's **Server Settings** page, and from the App in two places: an entry near the top of the server's own page, and a **Monitor settings** tab covering every agent at once. The tab is off by default; turn it on where the tab bar is arranged. Both reach the same agent and write the same file. What is not editable this way is deliberate: the JWT secret, the database path and the `[remote_access]` switches stay in the file, so a panel password can never widen what the agent exposes.
