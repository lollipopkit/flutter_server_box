...
**なぜこの Store レイヤーなのか？**
- `AppDb` + Drift がリレーショナルでクエリ負荷の高いデータを扱う
- `PrefStore` が軽量なキー/バリュー永続化を担う
- `SQLCipher` がローカルデータベース内の機微な記録を保護する
- Store API が永続化の詳細を UI や業務ロジックから分離する

**ストア:**
- `SettingStore`: アプリの設定
- `HistoryStore`: 最近のコマンドとナビゲーション履歴
- `ServerStore`: サーバー構成
- `ContainerStore`: コンテナホストの対応付けと設定
- `SnippetStore`: コマンドスニペット
- `PrivateKeyStore`: SSH キー
- `ConnectionStatsStore`: 接続テレメトリと集計
