...
1. システムキーボードで入力できないキー(Esc、Tab、Ctrl/Alt の組み合わせ、記号)は、ターミナルの仮想キーボード行から送信する
2. **IME** 仮想キーでシステムキーボードの表示/非表示を切り替える
3. サードパーティ製キーボードの不具合時は、システム標準のキーボードを試す
...
- ウォッチアプリを再起動してください。
- 設定変更後、数分待機してください。
- そのサーバーに monitor agent が設定されているか確認してください。ウォッチは
  agent があるサーバーしか表示できません。
