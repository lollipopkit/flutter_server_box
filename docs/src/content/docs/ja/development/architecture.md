...
- 既存のモデルパターンに従ってください。多くの store は `hive_ce` を使いますが、一部の追跡済みモデルは引き続き `@HiveType` と `@HiveField` を明示します
- 型アダプターの自動生成
- 永続的なキーバリューストレージ
...
## ステータス解析:共有 Rust ライブラリ

サーバーステータス解析(CPU、メモリ、ディスク、ネットワーク、温度、GPU、SMART など)は
Rust crate `crates/sbm_parser` に一度だけ実装され、アプリは flutter_rust_bridge
(`crates/sbm_ffi`、生成された Dart は `lib/src/rust/`)経由で利用します。
サーバーサイドの monitor は同じ crate を直接利用するため、両端の解析は常に一致します。
パーサーは純関数で、生のカウンタのみを返します。差分・移動窓の計算(例:ネットワーク速度)も
純関数であり、FFI 境界を可変状態が越えることはありません。
