...
访问一台服务器有两种方式：通过 **SSH**，或通过运行在它上面的 **[monitor agent](https://github.com/lollipopkit/flutter_server_box/tree/main/monitor)**（App 经 HTTP 与之通信）。SSH 无需在服务器上安装任何东西，且支持全部功能；agent 适用于不便暴露 SSH 端口的主机，并且自己保存历史数据，因此 App 连上之前图表就已经有数据。
