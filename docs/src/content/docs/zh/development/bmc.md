---
title: BMC 实现
description: Redfish 集成、资源发现和硬件行为
---
...
本文记录 Redfish 实现及已观察到的硬件行为。BMC 使用指南见 [BMC（Redfish）](/docs/zh/advanced/bmc/)；功能范围和设计依据见 [BMC 概述](/docs/zh/principles/bmc/)。

SSH 和 Monitor agent 都要求主机操作系统运行：SSH 需要 `sshd`，Monitor agent 需要主机上的进程。主机断电、卡死或重启时，二者通常只能报告连接失败。
