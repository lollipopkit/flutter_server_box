...
### 本地存储：AppDb/SQLCipher + PrefStore

- **AppDb + Drift**：用于结构化数据的关系型本地持久化
- **SQLCipher**：在落盘时加密敏感数据库表
- **PrefStore**：负责轻量级键值设置与历史记录
- **Store API**：以统一接口隐藏底层存储实现

当前仓库约定已将 `lib/data/store/` 定义为本地存储抽象而非 Hive-only；旧版 Hive 数据会通过 `HiveToSqliteMigrator` 迁移到基于 AppDb/SQLCipher 的实现。
