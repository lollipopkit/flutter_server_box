...
Server Box 是连接你所配置服务器的客户端。它没有用户账号，也没有由开发者运营、用于中转 SSH、SFTP 或 Monitor 流量的服务。这些连接会从 App 直接发往你选择的目标。

App 还会为诊断和你使用的功能发起独立请求。下文会说明这些请求的目标和内容。Server Box 不包含广告，不使用广告标识符，也不会跨 App 追踪。
...
| App 日志 | 仅保存在本机日志文件中；为便于排查问题，会保留当前日志和上一份日志 |
| Agent 对话 | 本机数据库；其中可能包含 prompt、模型回复、命令输出、选中的终端文本和文件内容 |
| 普通终端输出 | 保存在活动会话中；会话结束后，终端本身不会继续保存这些内容 |

Agent 对话不会加入备份，也不会通过设备同步发送，但在你从对话历史中删除前，会保留在本设备上。Agent 对话仍可能被发送给你配置的 AI provider；详见[AI 请求](#ai-请求)。

备份只在你主动请求时创建。根据选项和备份格式，备份可能包含服务器设置和凭据、私钥、代码片段、端口映射、容器设置、连接历史以及 App 设置。包含 App 设置时，其中也包括配置的 AI endpoint、model 和 API key。Agent 对话和设备本地的 bookkeeping 不会包含在内。
...
自动诊断是 Server Box 唯一会发送给开发者的数据。第一次自动上传前，引导页会先展示选择；之后可以随时在 **设置 → 诊断数据** 中更改。

| 级别 | 自动发送的内容 |
|---|---|
| **不发送** | 不发送任何内容。本机日志仍保留在设备上，你仍可以准备手动报告。 |
| **基本信息** | 被捕获的 Dart 错误和其他已报告错误，以及构建信息和 App 主动记录、用于解释错误上下文的诊断面包屑。App 正常运行时不会持续发送数据。 |
| **完整信息** | 基本信息的全部内容，外加可用的性能追踪，以及 App 运行期间的粗粒度功能使用计数。 |

Android 默认值为**不发送**。iOS、macOS、Linux 和 Windows 默认值为**基本信息**。在支持自动诊断的构建中，第一次自动上传前都会显示引导页。

改为**不发送**会立即移除上传 sink，不会等到下次启动。
...
- 结构化诊断面包屑，例如使用的连接方式、终端或文件 backend 类型，以及某项操作是否成功

面包屑使用固定的 action 名称，并对服务器相关值使用粗粒度信息或脱敏值。它们不会把终端输出、文件内容、密码或私钥作为诊断字段发送。App 自身的普通日志流在任何级别下都不会上传。

错误信息和调用栈来自底层 library，可能包含 App 没有主动创建的文本，因此不能保证每个异常都不包含服务器细节。手动报告发布前请先阅读，不要把自动诊断理解为任意异常文本都已匿名化。

平台在下次启动时提供的 native crash、ANR 或 hang 信息会写入本机日志和手动报告；Server Box 不会通过原生 crash handler 自动上传这些信息。
...
只有在构建版本配置了 analytics endpoint 时，功能使用事件才会发送。App 会使用随机生成的 128-bit 安装标识符，让同一次安装的事件保持顺序，并让实验分组在多次启动之间保持稳定。该标识符只在选择**完整信息**时存在：开启完整信息时创建，离开完整信息时删除，并且保存在备份文件之外。它不由设备标识符、账号或硬件信息推导而来。

完整信息启用且构建版本配置了实验时，App 可能会询问本次安装属于哪个分组。实验只会改变文案、顺序或默认值，不会决定某项功能是否存在、保存什么数据或如何连接服务器。

## 脱敏和手动崩溃报告

结构化诊断面包屑在交给 reporting sink 前就已处理为可发布的形式。它们使用固定的 action 名称，并用服务器相关值的替代值代替直接记录服务器名称、地址或用户名。

手动报告的处理方式不同。发生崩溃后，Server Box 可以显示上一运行的日志和平台提供的 crash 或 hang trace，并在能够识别时替换已配置的服务器名称、SSH/Monitor 地址和用户名，然后将结果复制到剪贴板并打开 GitHub issue 页面。它不会自动发布：是否复制或提交由你决定，你可以先阅读完整内容。这条路径在所有诊断级别下都可用，包括**不发送**。
...
API key 保存在本机加密的 App 数据库中，并作为 bearer credential 发送到你配置的 endpoint。选择在备份中包含 App 设置时，endpoint、model 和 API key 也会包含其中。Server Box 不会通过开发者的服务中转 AI 请求。
...
| 备份或同步 | iCloud、你的 WebDAV server 或 GitHub Gist | 创建、恢复或同步备份时；携带选定的备份文件 |
| 检查更新 | `api.github.com` | 开启自动检查更新后，在启动时发起 |
| Linux userland manifest | `github.com` | 启动时或刷新 manifest 时发起；携带版本元数据和签名 |
| Linux userland 镜像 | 经验证的 manifest 指定的 distribution mirror | 安装或更新本机 Linux 环境时发起；下载的镜像会按 manifest 中的 digest 校验 |
...
错误报告和性能追踪会发送到开发者运营的 Sentry-compatible server：`sentry.lollipopkit.com`。完整信息下的功能使用事件只会发送到构建版本提供的 analytics endpoint；没有配置该 endpoint 的构建版本不会发送功能使用事件。
...
App Store 和 Google Play 版本还可能受到平台自身上报机制的影响。Apple 和 Google 按照各自的政策收集相关信息，这些信息不是由 ServerBox 通过上述诊断服务收集的。详见 [Apple 隐私政策](https://www.apple.com/legal/privacy/) 和 [Google 隐私政策](https://policies.google.com/privacy)。F-Droid 版本没有商店提供的崩溃上报渠道，这也是其默认诊断级别为**不发送**的原因之一。
