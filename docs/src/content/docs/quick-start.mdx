...
Use Monitor agent instead when opening an SSH port is inconvenient:
...
A server using Monitor agent can show history collected before the App connected. An SSH-only server starts its local history when the App begins sampling.
