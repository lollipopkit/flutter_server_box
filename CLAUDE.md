...
- **A storage migration keeps a test that runs forever, fed by bytes the release being migrated *from* actually wrote.** It gets one pass over a user's records and is not repeatable — once the marker is written the old data is never read again — so a bug there is silence, not a crash. Seeding through the current adapters proves only that today's code agrees with itself.
  ...
  - This is not theoretical. `hive_import_test.dart` seeded `Spi` through the current adapter — typeId 15, SSH nested — while 1466 wrote typeId 3 flat, so the `SpiLegacyAdapter` → `_toSpi` path every upgrading install takes had no coverage. The fixture's first run then found that `PortForwardConfig`, the one freezed model with no `.g.dart` and so no generated `toJson`, was being dropped on every save as well as on import: `SqliteStore` encodes with `(value as dynamic).toJson()` and `set` returns `false` rather than throwing.
