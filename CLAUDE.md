...
- `crates/sbm_parser/` — shared status parser, the single source of truth for the command manifest and parsing; used by the app via FFI and by the server-side monitor. Locked by `tests/dart_compat.rs`.
  ...
  - **The user's custom commands are a third function, `SbCustom`, and a third cadence.** They are arbitrary shell a user typed with a 5-second timeout each; inside `SbStatus` a machine polled every three seconds ran all of them every three seconds and the built-in readings waited on the slowest. The app's interval is `Stores.setting.customCmdInterval`, rounded *up* to a whole number of poll intervals because the run happens on a poll and nowhere else (`effectiveCustomCmdSeconds`, shown in the settings entry so 7-against-3 does not silently mean 9). The monitor runs it on its extended cycle. Each side caches the last output and replays it (`_customRaw`, `refreshed_custom_cmds`), so a poll in between still carries the sections.
    - **The directory is the only copy and every client edits the same set**, so a save is compare-and-swap: `read_custom_cmds_script` prints a fingerprint, `install_custom_cmds_script` takes it as `expect` and answers `SrvBoxCusCmdConflict` rather than replacing. A successful install prints the new one, so the editor needs no second round trip and leaves no window for another save to land in. The monitor's HTTP API does the same over its own (SHA-256) fingerprint; the two never meet, since each client checks against what it read itself.
    - **Only the paths that write repair an interrupted install.** Installing moves the old directory aside and the new one into place; a connection dropped between those two leaves `$d.bak` and no `$d`. The reader and the installer move it back; `SbCustom` does not, and runs nothing — a poll that repairs state is a poll with a new way to fail, and the `.bak` fallback that used to be there meant every client silently ran a set nobody was editing.
...
**A tab whose subject is one of a standing set uses `SbPaneList`, not `AdaptivePanes.detail`.** A set that is always there — every server, every open terminal — is a rail, and `AdaptivePanes.surface` is the constructor for one: a narrow window keeps the *surface* and drops the rail, where `.detail` keeps the list and pushes. Reach for `.detail` only where the list is a history of records that the subject is made *out of*, which is the benchmark tab and not much else. `view/page/pkg/tab.dart` is the worked example of the first; `view/page/ssh/tab.dart` is what both are modelled on.

**The rail is compact, and the full-width list is not.** `listBuilder` is told `split` so it can be two things, and it has to be: a column at rail width is an index — you read down it for a name — so it uses `SideBarTile` (with `SideBarActions` above it for search), and a `CardX` per row there spends most of the width on its own edges. At full width the same rows are cards, where there is room for the subtitle a rail line leaves out. The rail supplies its own `Material` (`type: transparency`) rather than borrowing the home page's `Scaffold`: its rows are ink responses, and a column that only works under one particular ancestor fails as a red screen rather than as a compile error.
...
**Whenever the rail is not on screen, the bar's top left is the switcher.** That is the terminal tab's bar, in shape as well as in detail: `SessionSwitcherLabel` on the left — icon, name, chevron — saying which of the set is on screen and opening the rest, with the other ways out as `Btn.icon` actions on the right at **18pt**. The chevron is what makes "one of several" readable at all; a plain `CustomAppBar` title says "this page" and a bare icon in the `leading` slot says nothing.

It applies to any narrow layout showing one of the set, not only to a column the tab builds itself — a surface that replaced the list has no route to pop back to, so without the switcher a phone is left on one machine with no way to another. What the switcher opens is the same list widget the rail draws, raised as a sheet, so the counts and the search come with it and choosing from it is the same act as reading it.

**It goes in the bar, not in `AppBar.leading`.** That slot is one toolbar height wide and a switcher is a name; it overflows by a few pixels and the failure is a yellow-striped box, not an error anything catches. The shape is a `PreferredSize` of `SessionTabBar.height` holding a `Row`: the switcher in an `Expanded`, the actions after it. The page owns its own actions in that row — the tab cannot supply them without duplicating what the page knows.

`SessionSwitcherLabel` is `Align`ed to the start, so its ink is only as wide as its label. A widget test that taps the switcher by type taps the empty space beside it; tap its text.

**A tab's set has to be reachable before anything is chosen.** With two columns that is the rail. With one it is the surface: null selection is a real state, not an unset one, and what it draws is the list. Deciding the selection for the user instead — first server, last used — takes the overview away from the layout that has nowhere else to put it.
