...
- **A privacy manifest is validated on upload and by nothing before it.** App Store Connect parses `PrivacyInfo.xcprivacy` strictly; `plutil -lint` (CoreFoundation, lenient) and `package:xml` both accept things it rejects. XML forbids `--` inside a comment, both those parsers allow it, and a comment added to `ios/Runner/` and `macos/Runner/` in `fa6043f3` carried it — the same prose in both, so both platforms were rejected together with `ITMS-91056: Invalid privacy manifest`, which names a path and nothing else. `xmllint --noout` is the one local tool that says so.
  - `test/privacy_manifest_test.dart` covers every checked-in manifest: no `--` in a comment, plus keys, values and reason codes against **Xcode's own allowable-value list**, read out of `/Applications/Xcode.app/Contents/PlugIns/DVTCorePlistStructDefs.dvtplugin/Contents/Resources/DVTCorePlistStructDefs.xcplugindata` (`plutil -convert xml1`). That file is the authority for the ITMS-91054/91055/91056 family and pairs each API category with the reason codes valid *for it*.
  - Three manifests ship (`ios/Runner/`, `ios/StatusWidget/`, `ios/WatchApp/`) plus `macos/Runner/`. `ios/PrivacyInfo.xcprivacy` is referenced by no target and reaches no bundle — editing it does nothing. TODO: delete.
